...
a_n = {}
for i in pa:
  if int(i[1]) not in a_n:
    a_n[int(i[1])] = []
  a_n[int(i[1])].append(int(i[0]))

keys = sorted(a_n.keys())
a_n = [np.array(a_n[i]) for i in keys]
a_n = np.array(a_n, dtype=object)  # 修改此行以创建对象数组
...
p_n = {}
for i in pa:
  if int(i[0]) not in p_n:
    p_n[int(i[0])] = []
  p_n[int(i[0])].append(int(i[1]))

keys = sorted(p_n.keys())
p_n = [np.array(p_n[i]) for i in keys]
p_n = np.array(p_n, dtype=object)  # 修改此行以创建对象数组
