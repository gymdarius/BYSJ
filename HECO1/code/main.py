...
def perform_clustering_analysis(features, true_labels, dataset_name):
    print("执行聚类分析与评估...")
    print(f"原始数据维度 - 特征: {features.shape}, 标签: {true_labels.shape}")
    
    # 加载电影-导演关系以获取正确的电影节点索引
    md_path = "../data/imdb/md.txt"
    try:
        with open(md_path, 'r') as f:
            md_relations = [line.strip().split() for line in f]
        
        # 获取电影节点索引
        movie_indices = np.array([int(relation[0]) for relation in md_relations])
        
        # 确保索引在有效范围内
        valid_mask = movie_indices < features.shape[0]
        if not np.all(valid_mask):
            print(f"警告: 过滤掉了 {np.sum(~valid_mask)} 个超出范围的索引")
            movie_indices = movie_indices[valid_mask]
            true_labels = true_labels[valid_mask] if true_labels.ndim == 1 else true_labels[valid_mask, :]
        
        # 使用电影-导演关系的电影索引选择对应的嵌入向量
        features = features[movie_indices]
        print(f"调整后数据维度 - 特征: {features.shape}, 标签: {true_labels.shape}")
        
        # 确保维度匹配
        min_size = min(len(features), len(true_labels))
        if len(features) != len(true_labels):
            print(f"警告: 特征数量({len(features)})与标签数量({len(true_labels)})不匹配，截取到 {min_size}")
            features = features[:min_size]
            true_labels = true_labels[:min_size] if true_labels.ndim == 1 else true_labels[:min_size, :]
    except Exception as e:
        print(f"加载电影-导演关系时出错: {e}")
        print("继续使用原始特征...")
    
    # 确保标签格式正确（如果需要，将独热编码转换为类索引）
    ...
    plt.ylim(0, 1.1)  # NMI和ARI的范围均为[0,1]
    plt.title('聚类评估指标')
    plt.ylabel('得分')
...
def perform_tsne_analysis(features, labels, dataset_name, n_components=2):
    start_time = time.time()
    print(f"原始数据维度 - 特征: {features.shape}, 标签: {labels.shape}")
    ...
    try:
        ...
        if not np.all(valid_mask):
            print(f"警告: 过滤掉了 {np.sum(~valid_mask)} 个超出范围的索引")
            movie_indices = movie_indices[valid_mask]
            labels = labels[valid_mask] if labels.ndim == 1 else labels[valid_mask, :]
        
        # 使用电影-导演关系的电影索引选择对应的嵌入向量
        features = features[movie_indices]
        print(f"调整后数据维度 - 特征: {features.shape}, 标签: {labels.shape}")
        
        # 确保维度匹配
        min_size = min(len(features), len(labels))
        if len(features) != len(labels):
            print(f"警告: 特征数量({len(features)})与标签数量({len(labels)})不匹配，截取到 {min_size}")
            features = features[:min_size]
    ...
    if len(features) > 10000:
        print(f"数据集较大 ({len(features)} 样本)。抽样到10000个样本进行t-SNE分析。")
    ...
    end_time = time.time()
    print(f"t-SNE分析在 {end_time - start_time:.2f} 秒内完成")
    print(f"数据中发现 {len(np.unique(labels_cat))} 个不同的类")
