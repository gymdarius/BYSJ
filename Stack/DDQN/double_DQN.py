...
class Replay_Memory():
    ...
    # A simple (if not the most efficient) was to implement the memory is as a list of transitions.
        ...
        self.state = torch.empty(self.memory_size, state_size, dtype=torch.float, device=device)
        self.action = torch.empty(self.memory_size, action_size, dtype=torch.long, device=device)
        self.reward = torch.empty(self.memory_size, dtype=torch.float, device=device)
        self.next_state = torch.empty(self.memory_size, state_size, dtype=torch.float, device=device)
        self.done = torch.empty(self.memory_size, dtype=torch.int, device=device)
        self.count = 0
        self.idx = 0
    ...
    # Appends transition to the memory.
        ...
        self.state[self.idx] = torch.as_tensor(state, device=device)
        self.action[self.idx] = torch.as_tensor(action, device=device)
        self.reward[self.idx] = torch.as_tensor(reward, device=device)
        self.next_state[self.idx] = torch.as_tensor(next_state, device=device)
        self.done[self.idx] = torch.as_tensor(done, device=device)
