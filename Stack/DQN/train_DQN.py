...
import tqdm
import argparse
from DQN import Replay_Memory, DQN, device
import math
import pandas as pd
...
class Agent():
    ...
    def train(self, num_step):
        ...
        state = torch.tensor(self.env.reset(), device='cpu').view(1, -1).float().to(device)
        ...
        for i in range(num_step):
            ...
            # Keep CPU tensors for environment interaction
            transition = [state.cpu().numpy()[0], 14*index+action, reward, next_state, done]
            self.replay_memory.add(transition)
            mini_batch = self.replay_memory.sample_batch(self.batch_size) # Moves tensors to device in sample_batch
            ...
            if self.c % self.report_freq == 0:
                ...
                print(f"Step: {self.c}, Reward mean: {reward_mean:.2f}, Reward std: {reward_std:.2f}, Max_height mean: {max_height_mean:.2f}, Max_height std: {max_height_std:.2f}, Bumpiness mean: {bumpiness_mean:.2f}, Bumpiness std: {bumpiness_std:.2f}, Epsilon: {eps:.3f}, Loss: {total_loss/self.report_freq:.3f}")
                total_loss = 0
            ...
            if not done:
                state = torch.tensor(next_state, device='cpu').view(1, -1).float().to(device)
            else:
                state = torch.tensor(self.env.reset(), device='cpu').view(1, -1).float().to(device)
...
def test(env_name, model, model_file=None):
    ...
    with torch.no_grad():
        ...
        for _ in range(test_trial):
            state = torch.tensor(env.reset(), device='cpu').view(1, -1).float().to(device)
            ...
            while True:
                ...
                next_state = torch.tensor(next_state, device='cpu').view(1, -1).float().to(device)
                rewards += reward
                state = next_state
                if done:
                    ...
                    total_bumpiness.append(virance.item())
                    break
...
def parse_arguments():
    ...
    parser.add_argument('--gpu', dest='gpu', type=int, default=1, help='Use GPU (1) or CPU (0)')
...
def main(args):
    ...
    # Override device if GPU is explicitly disabled
    global device
    if args.gpu == 0:
        device = torch.device("cpu")
        print(f"GPU disabled by user. Using CPU instead.")
    else:
